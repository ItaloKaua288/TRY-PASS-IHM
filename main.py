...
class Game:
    def __init__(self):
        ...
        self.game_view = game_view.GameView(self.screen, self.assets, self)

        self.main_menu_controller = main_menu_controller.MainMenuController(self.main_menu_view, self)
        self.game_controller = game_controller.GameController(self.game_view, self)

        self.level_select_view = level_select_view.LevelSelectView(self.screen, self.assets, self)
        self.leve_select_controller = level_select_controller.LevelSelectController(self.level_select_view, self.assets, self)
    ...
    def run(self):
        while self.current_game_state != config.GameStateMap.QUIT:
            ...
            if self.current_game_state == config.GameStateMap.MAIN_MENU:
                self.main_menu_controller.handle_events(events)
